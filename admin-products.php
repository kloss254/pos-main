...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["add_product"])) {
    ...
    $stmt = $conn->prepare("INSERT INTO products (product_name, price, stock, image, tax) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("siisi", $product_name, $price, $stock, $imagePath, $tax);
    $stmt->execute();
    $stmt->close();
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["update_product"])) {
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        $imagePath = $targetDir . $imageName;
        move_uploaded_file($image['tmp_name'], $imagePath);
        $conn->query("UPDATE products SET product_name='$product_name', price=$price, stock=$stock, tax=$tax, image='$imagePath' WHERE id=$id");
    } else {
        $conn->query("UPDATE products SET product_name='$product_name', price=$price, stock=$stock, tax=$tax WHERE id=$id");
    }
}
